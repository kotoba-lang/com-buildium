(ns buildium.main
  "Kotodama WASM entrypoint for the Buildium clean-room actor (L4) — Clojure port.
  ...
  facts (`buildium.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "buildium")
(def tier "L4")
...
(def entity-specs
  [{:entity "Listing"     :plural "listings"     :id-prefix "buildium_lis"
   ...
   {:entity "Tenant"      :plural "tenants"       :id-prefix "buildium_ten"
...
(defn emit-facts
  "EAVT facts for one record: {\"buildium.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "buildium-compat" :tier tier :entities entities} 200])
...
(defn start! [] :buildium-compat/ready)
